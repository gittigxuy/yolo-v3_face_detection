...
val_bbx_file = 'wider_dataset/wider_face_split/wider_face_val_bbx_gt.txt'
train_bbx_file = 'wider_dataset/wider_face_split/wider_face_train_bbx_gt.txt'

val_data_folder = 'wider_dataset/WIDER_val'
train_data_folder = 'wider_dataset/WIDER_train'

out_file = 'wider_dataset/WIDER_val.txt'
...
def generate_train_file(bbx_file, data_folder, out_file):
    ...
    for data_line in data_lines:
        ...
        if sub_count >= 2:
            ...
            x_min = n_list[0]
            y_min = n_list[1]
            x_max = str(int(n_list[0]) + int(n_list[2]))#[0]是x,[2]是w
            y_max = str(int(n_list[1]) + int(n_list[3]))#[1]是y,[3]是h
...
def write_line(file_name, line):
    ...
    with open(file_name, "a+") as fs:
        if type(line) is (tuple or list):#有多个候选框
            fs.write("%s\n" % ", ".join(line))
        else:#有单个候选框
            fs.write("%s\n" % line)
